...
function isReportData(v: unknown): v is ReportData {
  ...
  return (
    ...
    typeof r['insightsResult'] === 'object' && r['insightsResult'] !== null
  );
}
...
export default function RelatorioPage(): JSX.Element {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (isReportData(parsed)) { setData(parsed); } else { setMissing(true); }
    } catch { setMissing(true); }
  }, []);

  const generatedAt = data ? new Date(data.insightsResult.generatedAt).toLocaleString('pt-BR') : '';
    ...
    <main className="mx-auto max-w-5xl px-6 py-8">
      ...
      <div className="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-arbor-subtle mt-1">Gerado em {generatedAt}{horizon !== undefined ? ` · Horizonte: ${horizon === 0 ? 'hoje' : `${horizon} anos`}` : ''}</p>
        </div>
